...
class VideoConvertor {
    fileprivate var verbose: Bool = false
    fileprivate var downscale: Bool = false
    fileprivate var scale: Double = 0
    fileprivate var hscale: Double = 2.0
    fileprivate var vscale: Double = 1.0
    fileprivate var cut: Bool = false
    fileprivate var framecounter: Int = 0
    fileprivate var assetduration: Float64 = 0
    
    func convertVideo(inputFile : URL, outputFile: URL , codec: AVVideoCodecType, fformat: AVFileType, verbose: Bool, downscale: Bool, cut: Bool) async throws {
        self.verbose = verbose
        self.downscale = downscale
        self.cut = cut
        
        if verbose {
            Swift.print("inputfile:    ", inputFile)
            // Load the AVAsset
            Swift.print("outputfile:   ", outputFile)
            Swift.print("verbose:      ", verbose)
            Swift.print("downscale:    ", downscale)
        }
        let asset = AVAsset(url: inputFile)
        // Swift.print("asset ",asset.classDescription)
        ...
        if spacialCharacteristics.count == 0 {
            Swift.print("spatial test:  no spatial video detected")
            throw VideoReaderError.notSpacialVideo
        }
        else {
            Swift.print("spatial test:  spatial video detected")
        }
        ...
        let vw = VideoWriter(url: outputFile, width: Int(videoSize.width*self.hscale), height: Int(videoSize.height/self.vscale), orientation: orientation, sessionStartTime: CMTime(value: 1,  timescale: 30 ), isRealTime: false, queue: .main, codec: codec, fformat: fformat, verbose: self.verbose)!
        ...
        while let nextSampleBuffer = output.copyNextSampleBuffer() {
            ...
            if let leftEyeBuffer,
               ...
               case let .pixelBuffer(rightEyePixelBuffer) = rightEyeBuffer {
                ...
                Swift.print("adding frames:", String(format: "%.2f", CMTimeGetSeconds(time)), "sec ", String(format: "%.0f", Float(time.value)/Float(duration.value)*100), "%", terminator: "\r")
                framecounter += 1
                if (cut && (framecounter > 50)) { break }
    ...
    func joinImages( leftImage:CIImage, rightImage:CIImage) -> CIImage {
        ...
        if self.downscale {
            self.scale = 4.0 //orig 4
        }
        else {
            self.scale = 2.0 //orig 4
        }
            
        let imageWidth = left.size.width/scale + right.size.width/scale
        let imageHeight = left.size.height/scale

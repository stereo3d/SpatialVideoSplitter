...
class VideoWriter {
    ...
    fileprivate let codec: AVVideoCodecType
    fileprivate let fformat: AVFileType
    ...
    init?(url: URL, width: Int, height: Int, orientation: CGAffineTransform, sessionStartTime: CMTime, isRealTime: Bool, queue: DispatchQueue, codec: AVVideoCodecType, fformat: AVFileType, verbose: Bool) {
        ...
        if self.verbose{
            print("\ncreate video:  width=\(width) height=\(height)")
           // print("url             : \(url)")
        }
        
        //AVVideoCodecKey : AVVideoCodecType.h264, // or .hevc if you like
        let outputSettings: [String:Any] = [
            AVVideoCodecKey : self.codec, // or .hevc if you like
        ...
        // output the filetype
        //if verbose {Swift.print(self.fformat)}
        guard
            // .mp4 to mov
            let writer = try? AVAssetWriter.init(url: url, fileType: self.fformat ),
            writer.canAdd(input),
    ...
    func finish() async throws -> AVAsset? {
        ...
        //if verbose {print("VideoWriter: calling writer.finishWriting()")}
        ...
        if verbose {print("success:       write video complete, duration=\(duration)")}
        return asset
    }
}
